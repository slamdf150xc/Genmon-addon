...
### Define config files ###
genmqttconf="/etc/genmon/genmqtt.conf"
genmonconf="/etc/genmon/genmon.conf"
mymailconf="/etc/genmon/mymail.conf"
genloaderconf="/etc/genmon/genloader.conf"
...
if [ "${enable_mqtt}" == true ]; then
    ...
    sed -i "s/flush_interval =/flush_interval = 60/g" $genmqttconf
...
/opt/genmon/startgenmon.sh start && tail -F /var/log/genmon.log